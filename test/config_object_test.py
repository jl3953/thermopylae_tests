...
class ConfigObjectTest(unittest.TestCase):
  ...
  def test_generate_ini_filename(self):
    ...
    self.assertEqual(
      "/usr/local/temp/go/src/github.com/cockroachdb/cockroach/tests/config/uniqueprefix_suffix.ini",
      ini)

  def test_enumerate_workload_nodes(self):
    workload_nodes = config_object.ConfigObject.enumerate_workload_nodes(1, 6)
  ...
  def test_enumerate_warm_nodes(self):
    warm_nodes = config_object.ConfigObject.enumerate_warm_nodes(4, 1, 6)
